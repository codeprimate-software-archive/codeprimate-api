 ...
 * @see org.codeprimate.process.ProcessConfiguration
 * @since 1.0.0
 */
@SuppressWarnings("unused")
public class ProcessWrapper implements Initable {
      ...
      @Override public void run() {
        if (isRunning()) {
          ...
          try {
            ...
            FileSystemUtils.close(inputReader);
          }
        }
      }
    };
  ...
  protected Thread newThread(final String name, final Runnable task) {
    Assert.legalArgument(StringUtils.hasText(name), "The name of the Thread must be specified!");
  ...
  public String readLogFile(final File log) throws IOException {
    return FileSystemUtils.read(log);
  }
  ...
  public void waitFor(final long milliseconds) {
    ThreadUtils.waitFor(milliseconds).checkEvery(500).on(new CompletableTask() {
