...
public class LinkIndex implements Iterable<Link> {
  ...
  public LinkIndex addAll(final Link... links) {
    Assert.notNull(links, "The array of Links being added to this index cannot be null!");
    return addAll(Arrays.asList(links));
  }

  public LinkIndex addAll(final Iterable<Link> links) {
    Assert.notNull(links, "The Iterable collection of Links being added to this index cannot be null!");
  ...
  public String toString() {
    ...
    for (Link link : this) {
      buffer.append(count++ > 0 ? ", " : StringUtils.EMPTY_STRING);
      buffer.append(String.format("%1$s - %2$s", link.getRelation(), link.toHttpRequestLine()));
    }
